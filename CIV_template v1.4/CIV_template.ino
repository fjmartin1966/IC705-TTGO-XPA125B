// Edit by KK7PZE
//  Since I have two of these device I wanted to have a common code base that functions differently depending on whether I was using/programming the Home unit or the Away unit
//    Setting Home = 1 will configure the device for home use.  Setting Home = 0 will set it for Away use
        int Home = 1;
...
void setup() {
  ...
  #ifdef useBT
    // mod by DL1BZ
    // KK7PZE changed the Bluetooth name to Home-XPA125B or Away-XPA125B
    Serial.print("Home: "); Serial.println(Home);
    if (Home == 1){
      // For Home use:
        Serial.println("Setting up for Home use");
        civ.setupp(true,"Home-XPA125B"); // BT identifier show at IC705
    }else if (Home == 0){
      // For Away use:
        Serial.println("Setting up for Away use");
        civ.setupp(true,"Away-XPA125B"); // BT identifier show at IC705
    }
  #else
