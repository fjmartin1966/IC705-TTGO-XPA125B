...
//Added by KK7PZE - Custom RF Gain setting not officially supported by CIVmasterLib
constexpr uint8_t CIV_C_RF_GAIN[] = {2,0x14,0x02};  // send / read RF Gain settings (0..255 == 0 .. 100%)
//Added by KK7PZE - Custom Pre-Amp setting not officially supported by CIVmasterLib
constexpr uint8_t CIV_C_PRE_AMP[] = {2,0x16,0x02};  // send / read Pre-Amp settings (0 = off, 1 = Pre-Amp 1, 2 = Pre-Amp 2). NOTE: 144 & 430 only support 0 and 1
...
void  CIV_getProcessAnswers() {
  ...
  //!//
  // put your code here in case you have added querys which shall be answered by the radio 
  // with data
